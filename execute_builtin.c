...
/*   Updated: 2023/01/25 14:50:28 by lloison          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	execute_builtin(t_cmd *cmd, char ***env, t_bool should_fork)
{
	...
	else if (compare_text_no_case("echo", cmd->args[0], 5) == 0)
		set_les(bi_echo(cmd->args, *env));
	else if (compare_text_no_case("pwd", cmd->args[0], 4) == 0)
		set_les(bi_pwd(cmd->args, *env));
	else if (compare_text_no_case("env", cmd->args[0], 4) == 0)
